...
class CompanyModel(models.Model):
    ...
    owner = models.ForeignKey("UserModel", on_delete=DO_NOTHING, null=False)
    ...
    business_id = models.CharField(max_length=100, null=False)
    
    def __str__(self):
        return f"('{self.business_name}', '{self.business_street_address}','{self.business_registered_country}','{self.business_registered_city}', '{self.business_registered_zipcode}', '{self.business_contact_number}', '{self.business_email}', '{self.business_fax_number}', {self.business_type}', {self.business_size}', {self.business_id}', {self.owner}', {self.timestamp}', )"
